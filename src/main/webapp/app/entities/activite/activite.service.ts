...
export class ActiviteService {
  ...
  protected convertDateFromClient(activite: IActivite): IActivite {
    const copy: IActivite = Object.assign({}, activite, {
      ...
      dateCreated: activite.dateCreated && activite.dateCreated.isValid() ? activite.dateCreated.format(DATE_FORMAT) : undefined
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((activite: IActivite) => {
        ...
        activite.dateCreated = activite.dateCreated ? moment(activite.dateCreated) : undefined;
      });
    }
    return res;
  }
