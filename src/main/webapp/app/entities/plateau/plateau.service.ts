...
export class PlateauService {
  ...
  protected convertDateFromClient(plateau: IPlateau): IPlateau {
    const copy: IPlateau = Object.assign({}, plateau, {
      dateCreated: plateau.dateCreated && plateau.dateCreated.isValid() ? plateau.dateCreated.format(DATE_FORMAT) : undefined,
      dateUpdated: plateau.dateUpdated && plateau.dateUpdated.isValid() ? plateau.dateUpdated.format(DATE_FORMAT) : undefined
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((plateau: IPlateau) => {
        plateau.dateCreated = plateau.dateCreated ? moment(plateau.dateCreated) : undefined;
        plateau.dateUpdated = plateau.dateUpdated ? moment(plateau.dateUpdated) : undefined;
      });
    }
    return res;
  }
