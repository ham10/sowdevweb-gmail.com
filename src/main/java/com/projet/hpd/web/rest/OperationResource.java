...
/**
 * REST controller for managing {@link com.projet.hpd.domain.Operation}.
 */
@RestController
@RequestMapping("/api")
public class OperationResource {
    ...
    public ResponseEntity<List<Operation>> getAllOperations(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("schemacomptable-is-null".equals(filter)) {
            log.debug("REST request to get all Operations where schemaComptable is null");
            return new ResponseEntity<>(operationService.findAllWhereSchemaComptableIsNull(),
                    HttpStatus.OK);
        }
