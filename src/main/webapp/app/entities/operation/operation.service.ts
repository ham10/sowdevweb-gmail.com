...
export class OperationService {
  ...
  protected convertDateFromClient(operation: IOperation): IOperation {
    const copy: IOperation = Object.assign({}, operation, {
      date: operation.date && operation.date.isValid() ? operation.date.format(DATE_FORMAT) : undefined,
      dateCreated: operation.dateCreated && operation.dateCreated.isValid() ? operation.dateCreated.format(DATE_FORMAT) : undefined,
      dateUpdated: operation.dateUpdated && operation.dateUpdated.isValid() ? operation.dateUpdated.format(DATE_FORMAT) : undefined,
      dateDeleted: operation.dateDeleted && operation.dateDeleted.isValid() ? operation.dateDeleted.format(DATE_FORMAT) : undefined
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((operation: IOperation) => {
        ...
        operation.dateCreated = operation.dateCreated ? moment(operation.dateCreated) : undefined;
        operation.dateUpdated = operation.dateUpdated ? moment(operation.dateUpdated) : undefined;
        operation.dateDeleted = operation.dateDeleted ? moment(operation.dateDeleted) : undefined;
      });
    }
    return res;
  }
